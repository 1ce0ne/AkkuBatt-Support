...
@bot.message_handler(func=lambda message: flag and message.content_type != 'photo')
...
def validate_rental_time(rental_time):
    try:
        ...
        # Проверка, что дата аренды не раньше текущего времени и не позднее чем через 30 дней
        if rental_datetime > now_time:
            return False  # Дата аренды раньше текущего времени
        elif rental_datetime < now_time - timedelta(days=30):
            return False  # Дата аренды позже чем через 30 дней

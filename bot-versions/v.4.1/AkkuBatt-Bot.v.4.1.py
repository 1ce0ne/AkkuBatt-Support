...
BUG_AND_FEEDBACK_ID = os.environ.get('BUG_AND_FEEDBACK_ID')
DATABASE = os.environ.get('DATABASE_PATH', 'Reports.db')
BUGS_DB = os.environ.get('BUGS_DB', 'BugReports.db')
FEEDBACK_DB = os.environ.get('FEEDBACK_DB', 'Feedback.db')
...
FeedbackAgreement = (
    ...
    '6. Подтверждение:\n'
    'Нажимая "Отправить", вы подтверждаете согласие с данными условиями ✅'
)
...
def start_bug_report(message):
    # Initialize bug report state for this user
...
def save_report(photo, scooter_number, phone_number, card_number, rental_time, description, user_id):
    ...
    try:
        ...
        # Пробрасываем исключение дальше для обработки в process_description
        raise e
    finally:
        if conn:
            conn.close()
